(() => {
    ...
    const save = () => {
        ...
        const msg = encodeURI('KNPS Macro: 예약 알림이 연동되었습니다.');
        const url = `https://api.telegram.org/bot${botToken}/sendMessage?chat_id=${chatId}&text=${msg}`;

        fetch(url).then(response => {
            if (response.status === 200) {
                localStorage.setItem('KNPS_MACRO::bot-token', botToken);
                localStorage.setItem('KNPS_MACRO::chat-id', chatId);
    ...
    const reset = () => {
        ...
        localStorage.removeItem('KNPS_MACRO::bot-token');
        localStorage.removeItem('KNPS_MACRO::chat-id');
